...
_acquire_success = IOSuccess(u'acquire success')
_acquire_failure = IOFailure(u'acquire failure')


def _use_success(inner_value):
    return IOSuccess(u'use success')


def _use_failure(inner_value):
    return IOFailure(u'use failure')
...
class _ReleaseFailure(object):
    ...
    def __call__(
        ...
        return IOFailure(u'release failure')
...
def test_full_typing():
    ...
    assert pipeline_result == IOSuccess(u'use success')
